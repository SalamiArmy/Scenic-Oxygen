...
def run(bot, keyConfig, chat_id, user, message, intention_confidence=0.0):
    ...
    if len(data[2]) >= 1:
        ...
        bot.sendMessage(chat_id=userWithCurrentChatAction, text=urlForCurrentChatAction +
                        '\nMight I add that I am ' + str(intention_confidence) + '% confident you wanted to know this.' if intention_confidence > 0.0 else '', disable_web_page_preview=True)
    else:
        wikiUrl = \
        ...
        if len(data[2]) >= 1 and not data[2][0] == '':
            bot.sendMessage(chat_id=chat_id, text=(user + ': ' if not user == '' else '') + \
                                      data[2][0] + '\nLink: ' + data[3][0] +
                        '\nMight I add that I am ' + str(intention_confidence) + '% confident you wanted to know this.' if intention_confidence > 0.0 else '',
